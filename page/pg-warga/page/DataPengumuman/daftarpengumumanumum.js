...
import Listpengumumanumum from './listpengumumanumum';
...
export default class daftarpengumumanumum extends React.Component {
  ...
  componentDidMount() {
    RefPengumuman.orderByChild("kd_penerima").equalTo(localStorage.getItem("no_kk")).on('value', snap => {
      const tasks = [];
      snap.forEach(shot => {
  ...
  render() {
    ...
    if (tasksLoading) {
      ...
      taskList = (
        ...
        <div className="active tab-pane" id="timeline">
          {orderedTasks.map(task => (
            <Listpengumumanumum key={task.key} task={task} />
          ))}
      ...
      taskList = <div className="TaskList-empty">Tidak Ada Pengumuman Umum</div>;
    }
    return taskList;
  }
}