...
import ListSurat from "./page/DataSurat/listSurat";
import { rootRef, RefKK, timeRef } from '../../db';
...
class Surat extends Component {
    ...
    constructor(props) {
        super(props);
        this.state = { idrt:'',no:'',search:'',kode:'',no:'', alert:'', tampilalert:'hide',tasks: [], tasksLoading: true,jenis_surat:'', nama_penerima:localStorage.getItem('namarw')};
      ...
      handleChangeJenis = event => {
        ...
        //     if(shot.val().jenis_surat === (event.target.value === null? '' : event.target.value)){
  ...
  render() {
    const { displayPenerima, tasks, tasksLoading, search,tampilalert} = this.state;
    ...
    if(tampilalert === "hide"){
      if(this.state.jenis_surat.length && this.state.kode.length){
        button =
          <div>
          <button type="reset" className="btn btn-danger"  onClick={this.handleReset}>Reset</button><span></span>
