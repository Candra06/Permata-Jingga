...
export default class daftarwarga extends React.Component {
  ...
  handleChangeSub = event =>{
    ...
    if(event.target.value.length === 0){
      ...
      try{
        ...
        this.setState({subpilihan: event.target.value, showbtn:true, judulexcel:"Data Warga Berdasakan "+pilihanutama+" dengan nilai "+event.target.value+".xlsx", teksdetail:"Menampilkan data warga berdasarkan <b>RW "+ tmprw + " RT "+ event.target.value+"</b>", linkdown:"http://wargaku.id/exportdatawarga/p=rw&pilihan="+pilihanutama+"&nilairt="+event.target.value+"&nilairw="+localStorage.getItem("koderw")});
      }catch(error){
  ...
  render() {
    const { tasks, tasksLoading, pilihanutama, showbtn, subpilihan, arrayrw, arrayrt, judulexcel } = this.state;
        ...
        <div className="col-md-2">
          ...
          <div className="col-md-6">
            {/* <a target="_blank" href={linkdown}>
              <button className="btn btn-primary" onClick={this.exportdata} title="Export Data"><i className="fa fa-file-excel-o"></i></button>
            </a> */}
