...
export default class daftarwarga extends React.Component {
  ...
  arrayrw:[], tmprw:'', tmprt:'', arrayrt:[], judulexcel:'' };
  ...
  pencarian(dari, isi){
    try{
      RefWarga.orderByChild(dari).equalTo(isi).on('value', snap => {
        ...
        let no = 0;
        snap.forEach(shot => {
          if(shot.val().norw === localStorage.getItem("koderw") && shot.val().tipewarga === "Tambahan"){
  ...
  handleChangeSub = event =>{
    ...
    if(event.target.value.length === 0){
      ...
      try{
        ...
        this.setState({subpilihan: event.target.value, showbtn:true, teksdetail:"Menampilkan data berdasarkan <b>"+ pilihanutama + "</b> dengan nilai <b>"+ event.target.value+"</b>", linkdown:"http://wargaku.id/exportdatawarga/p=rw&pilihan="+pilihanutama+"&nilai="+event.target.value+"&np="+localStorage.getItem("koderw")});
      }catch(error){
      ...
      try{
        ...
        this.setState({subpilihan: event.target.value, showbtn:true, teksdetail:"Menampilkan data warga berdasarkan <b>RW "+ tmprw + " RT "+ event.target.value+"</b>", linkdown:"http://wargaku.id/exportdatawarga/p=rw&pilihan="+pilihanutama+"&nilairt="+event.target.value+"&nilairw="+localStorage.getItem("koderw")+"&np="+localStorage.getItem("koderw")});
      }catch(error){
  ...
  render() {
    const { tasks, tasksLoading, pilihanutama, showbtn, judulexcel, arrayrw, arrayrt, subpilihan } = this.state;
    let orderedTasks;
    ...
    if(showbtn){
      ...
      tampilbtn = (
        ...
        <div className="form-group col-md-12">
            <p>Detail : {ReactHtmlParser(this.state.teksdetail)} <b>{orderedTasks.length}</b></p>
        </div>
    </div>
      )
      hapusfilter = (
