...
import Listkk from './listpengeluaran';
import { Refpengeluaran } from './../../../../db';
import Listbln from "./../DataIuran/listitembulan";
import Workbook from 'react-excel-workbook';
export default class daftarpengeluaran extends React.Component {
  state = {
  ...
  tampildata= (xtipe)=>{
    let jmldata=0;
    Refpengeluaran.limitToFirst(this.state.items).orderByChild("nort").equalTo(localStorage.getItem("idrt")).on('value', snap => {
  ...
  loadMore=(xtipe)=> {
    ...
    if(xtipe === "semua"){
      Refpengeluaran.limitToFirst(number).orderByChild("nort").equalTo(localStorage.getItem("idrt")).on('value', snap => {
        const tasks = [];
        let no = 0;
        snap.forEach(shot => {
          no++
  ...
  handleChangeSubPilihan = event => {
    ...
    this.setState({ subpilihan: event.target.value, showbtn:true, judulexcel:'Data Pengeluaran Berdasarkan '+ this.state.pilihan + ' Dengan nilai ' + event.target.value});
  ...
  render() {
    ...
    if(pilihan === "perbulan"){
    ...
    }else if(pilihan === "keperluan"){
      orderedTasks = orderBy(
        tasks.filter(
          (ls) => {
            return ls.keperluan.indexOf(subpilihan)!==-1;
          }
        ), ['ordertime'], ['asc']
      );
      ...
      keyword = (
        <div className="col-md-3">
            <div className="form-group col-md-12">
                ...
                onChange={this.handleChangeSubPilihan}
                value={this.state.subpilihan}
                className="form-control" placeholder="Masukkan Keperluan"/>
              </div>
        </div>
      )
      ...
      orderedTasks = orderBy(
        tasks.filter(
          (ls) => {
            return ls.keterangan.indexOf(subpilihan)!==-1 || ls.keperluan.indexOf(subpilihan)!==-1 || ls.tanggal.indexOf(subpilihan)!==-1 || ls.jumlah_uang.indexOf(subpilihan)!==-1 || ls.nama_pembuat.indexOf(subpilihan)!==-1;
          }
        ), ['ordertime'], ['asc']
      );
    }
    ...
    if (tasksLoading) {
      ...
      taskList = (
        ...
        <div className="row">
          ...
          <div className="TaskList-empty">Tidak Ada Data Pengeluaran</div>
      </div>);
    }
