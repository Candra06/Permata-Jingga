...
export default class Listkk extends React.Component {
    ...
    tutupmodal2(){
      ...
      toggle2(txtidrt, txtalamat, txtno, txtidrw, txtkepala, txtkelurahan) {
        ...
        RefRW.on('value', snap => {
          ...
          RefRT.orderByChild("norw").equalTo(txtidrw).on('value', snap => {
            const arrayrt = [];
            snap.forEach(shot => {
      ...
      handleChangeNo = event => {
        ...
        let hitung = 0;
              RefKK.orderByChild("nokk").equalTo(event.target.value).on('value', snap => {
                snap.forEach(shot => {
                    if(shot.val().nokk != this.state.tmpnokk){
                    hitung++;
                    }else{
      ...
      handleChangeOptionRW = event => {
        this.setState({norw: event.target.value, nort:''});
  ...
  render() {
    if(this.state.redirect){
        return (<Redirect to="/Admin/ViewKK"/>)
     }
    const { task } = this.props;
    const { tampilalert, arrayrw, arrayrt, arraykelurahan, arrayselectkepala } = this.state;
  let cekalert;
