...
import { RefRW, rootRef, RefWarga, RefWargaTmp, RefUser, upRefWarga, RefRT } from './../../../../../db';
...
export default class Listwarga extends React.Component {
    ...
    this.state = {
        ...
        modal3: false,
        modal4: false,
        nama: '', nik: '', tempatlahir: '', tgl_lahir:'', nohp: '', url:'',
        pekerjaan: '', gender: '', email: '', agama:'', fb: '', wa: '', ig: '', twitter: '', kunci: '',
        pendidikan:'', status_nikah:'', kewarganegaraan:'', namaayah:'', namaibu:'', htgemail:0, htgnik:0, tmpemail: '', tmpnik: '',
        norw:'', nort:'', alamat:'', tasksrw:[], tasksrt:[],
        avatar: '',
        isUploading: false,
        progress: 0,
        avatarURL: ''
      ...
      this.handleChangeOptionRW = this.handleChangeOptionRW.bind(this);
      this.handleChangeOptionRT = this.handleChangeOptionRT.bind(this);
    }

    componentDidMount(){
        // localStorage.setItem('DataRTTambahan', "")
        // localStorage.setItem('DataRWTambahan', "")
        RefRW.on('value', snap => {
            const tasksrw = [];
            snap.forEach(shot => {
                tasksrw.push({ ...shot.val(), key: shot.key });
            });
            this.setState({ tasksrw });
          });
      }
      handleChangeOptionRW = (event) => {
        this.setState({norw: event.target.value});
        RefRT.orderByChild("norw").equalTo(event.target.value).on('value', snap => {
          const tasksrt = [];
          snap.forEach(shot => {
            tasksrt.push({ ...shot.val(), key: shot.key });
          });
          this.setState({ tasksrt });
        });
      }

      handleChangeOptionRT = (event) => {
        this.setState({nort: event.target.value });
      }
    ...
    toggle() {
      ...
      toggle2(xkey, xnama, xnik, xtempatlahi, xtgl_lahir, xnohp, xagama, xpekerjaan, xgender, xemail, xfb, xwa, xig, xtwitter, xpendidikan, xstatusnikah, xkewarganegaraan, xnamaayah, xnamaibu, xurl, xnorw, xstatus) {
        this.setState({
            nama: xnama, nik: xnik, tmpemail: xemail, tmpnik: xnik, tempatlahir: xtempatlahi, tgl_lahir:xtgl_lahir, nohp: xnohp, agama: xagama,
        pekerjaan: xpekerjaan, gender: xgender, email: xemail, fb: xfb, wa: xwa, ig: xig, twitter:xtwitter , kunci:xkey,
        pendidikan: xpendidikan, status_nikah: xstatusnikah, kewarganegaraan: xkewarganegaraan, namaayah: xnamaayah, namaibu: xnamaibu,
        alamat: xurl, norw: xnorw,nort:xstatus , 
          modal2: !this.state.modal2
        });
        try{
            
          RefRT.orderByChild("norw").equalTo(xnorw).on('value', snap => {
            const tasksrt = [];
            snap.forEach(shot => {
              tasksrt.push({ ...shot.val(), key: shot.key });
            });
    ...
    RefWarga.child(txtkunci).update({
      ...
      notify(type, kunci){
        return () => {
            ...
            if(this.state.norw === ""){
                toast.warn('RW tidak boleh kosong', {
                    position: toast.POSITION.TOP_RIGHT,
                  });
              }else if(this.state.nort === ""){
                toast.warn('RT tidak boleh kosong', {
                    position: toast.POSITION.TOP_RIGHT,
                  });
            ...
            toast.warn('Alamat tidak boleh kosong', {
                position: toast.POSITION.TOP_RIGHT,
                  ...
                  if(this.state.htgemail >= 1){
                    ...
                    }else if(this.state.htgnik >= 1 && this.state.nik != this.state.tmpnik){
                ...
                RefWarga.child(kunci).update({ nama: this.state.nama,
                    ...
                    this.toggle2()
                this.setState({  nama:'', nik: '', showalert: 'hide', showalertnik: 'hide', lihatbtn: '',
                teksalert:'', tempatlahir: '', tgl_lahir:'', nohp: '',
                pekerjaan: '', gender: '', email:'', fb: '', wa: '', ig: '', twitter: '', agama: '',
                pendidikan:'', status_nikah:'', kewarganegaraan:'', namaayah:'', namaibu:'', alamat:'',
                addwarga:false, norw:'', nort:''});
  ...
  render() {
    ...
    const { tasksrw, tasksrt } = this.state;
    ...
    return (
        ...
        <Modal isOpen={this.state.modal3} toggle={this.toggle3} frame position="top">
            ...
            <Button color="primary" onClick={() => this.toggle2(task.key, task.nama, task.nik, task.tempatlahir, 
                    task.tgl_lahir, task.nohp, task.agama
                    , task.pekerjaan, task.gender, task.email, task.fb, task.wa, task.ig, task.twitter,
                    task.pendidikan, task.status_nikah, task.kewarganegaraan, task.namaayah, task.namaibu, task.alamat
                    , task.norw, task.nort)}><i className="fa fa-pencil"></i></Button>
                ...
                <div className="form-control">
                            <div className="card-body">
                                ...
                                <div className="row col-12">
                                        ...
                                        <select className="form-control" value={this.state.norw} onChange={this.handleChangeOptionRW} style={{height: 3 + 'em'}}>
                                        ...
                                        <select className="form-control" value={this.state.nort} onChange={this.handleChangeOptionRT} style={{height: 3 + 'em'}}>
