...
export default class daftarkk extends React.Component {
  ...
  componentDidMount() {
    ...
    RefKK.limitToFirst(this.state.items).orderByChild("idrw").equalTo(localStorage.getItem("koderw")).on('value', snap => {
      let no = 0;
      snap.forEach(shot => {
  ...
  loadMore() {
    ...
    RefKK.limitToFirst(number).orderByChild("idrw").equalTo(localStorage.getItem("koderw")).on('value', snap => {
      const tasks = [];
      let no = 1;
      snap.forEach(shot => {
        if(shot.val().idrt === localStorage.getItem("idrt")){         
