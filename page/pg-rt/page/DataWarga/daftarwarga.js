...
export default class daftarwarga extends React.Component {
  ...
  arrayrw:[], tmprw:'', tmprt:'', judulexcel:'',
  gridnumber:0,
  ...
  pencarian(dari, isi){
    try{
      RefWarga.orderByChild(dari).equalTo(isi).on('value', snap => {
        ...
        let no = 0;
        snap.forEach(shot => {
          if(shot.val().norw === localStorage.getItem("koderw") && shot.val().nort === localStorage.getItem("idrt")){
          no++;
          tasks.push({ ...shot.val(), key: shot.key, nomer: no });
          }
        });
      ...
      try{
        ...
        RefWarga.orderByChild("norw").equalTo(localStorage.getItem("koderw")).on('value', snap => {
          ...
          snap.forEach(shot => {
            ...
            if(ageFromDate === event.target.value && shot.val().nort === localStorage.getItem("idrt")){
                no++;
                tasks.push({ ...shot.val(), key: shot.key, nomer: no });
  ...
  render() {
    const { tasks, tasksLoading, pilihanutama, showbtn, subpilihan, judulexcel } = this.state;
    let orderedTasks;
