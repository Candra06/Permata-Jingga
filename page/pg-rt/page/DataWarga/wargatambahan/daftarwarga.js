...
import { RefWarga,  RefRT } from './../../../../../db';
import ReactHtmlParser from 'react-html-parser';

export default class daftarwarga extends React.Component {
  state = {
    tasks: [], tasksLoading: true, pilihanutama:'',
    subpilihan:'', 
    linkdown:'', 
    showbtn:false, 
    teksdetail:'', 
  ...
  componentDidMount() {
    RefWarga.orderByChild("norw").equalTo(localStorage.getItem("koderw")).on('value', snap => {
      ...
      let no = 1;
      snap.forEach(shot => {
        if(shot.val().tipewarga === "Tambahan" && shot.val().nort === localStorage.getItem("idrt")){
        no++;      
        }
      });
    ...
    RefWarga.limitToFirst(this.state.items).orderByChild("norw").equalTo(localStorage.getItem("koderw")).on('value', snap => {
      const tasks = [];
      let no = 0;
      snap.forEach(shot => {
        if(shot.val().tipewarga === "Tambahan" && shot.val().nort === localStorage.getItem("idrt")){
  ...
  loadMore() {
    ...
    RefWarga.limitToFirst(number).orderByChild("norw").equalTo(localStorage.getItem("koderw")).on('value', snap => {
      const taks = [];
      let no = 1;
      snap.forEach(shot => {
        if(shot.val().tipewarga === "Tambahan" && shot.val().nort === localStorage.getItem("idrt")){
          taks.push({...shot.val(), key:shot.key, nomer:no})
          no++;
          }
  ...
  pencarian(dari, isi){
    try{
      RefWarga.orderByChild(dari).equalTo(isi).on('value', snap => {
        ...
        let no = 0;
        snap.forEach(shot => {
          if(shot.val().norw === localStorage.getItem("koderw") && shot.val().tipewarga === "Tambahan" &&  shot.val().nort === localStorage.getItem("idrt")){
  ...
  handleChangePilihan = event =>{
    ...
    this.tampildata();
    this.setState({pilihanutama: event.target.value, subpilihan:''});
 
  }
 
  handleChangeSub = event =>{
    const { pilihanutama} = this.state;
    ...
    if(pilihanutama === "usia"){
      try{
        ...
        this.setState({subpilihan: event.target.value, showbtn:true, teksdetail:"Menampilkan data berdasarkan <b>"+ pilihanutama + "</b> dengan nilai <b>"+ event.target.value+"</b>", linkdown:"http://wargaku.id/exportdatawarga/p=rt&pilihan="+pilihanutama+"&nilai="+event.target.value+"&np="+localStorage.getItem("idrt")});
      }catch(error){
  ...
  render() {
    const { tasks, tasksLoading, pilihanutama, showbtn, judulexcel, subpilihan } = this.state;
    let orderedTasks;

    let taskList, keyword, tampilbtn, hapusfilter;
    if(showbtn){
    ...
    if (tasksLoading) {
      ...
      taskList = (
        ...
        <div className="row">
          ...
          {tampilbtn}
      <div className="TaskList-empty">Tidak Ada Data Warga Tambahan</div>
      </div>
    );
    }
