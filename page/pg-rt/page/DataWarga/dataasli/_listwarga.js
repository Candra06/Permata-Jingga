...
export default class Listwarga extends React.Component {
    ...
    toggle() {
      ...
      toggle2(xkey, xnama, xnik, xtempatlahi, xtgl_lahir, xnohp, xurl, xagama, xpekerjaan, xgender, xemail, xstatus, xfb, xwa, xig, xtwitter, xpendidikan, xstatusnikah, xkewarganegaraan, xnamaayah, xnamaibu) {
        this.setState({
        ...
        pendidikan: xpendidikan, urlfoto: xurl, status_nikah: xstatusnikah, kewarganegaraan: xkewarganegaraan, namaayah: xnamaayah, namaibu: xnamaibu,
      ...
      deleteTask = (txtemail) => {
        ...
        Action("deleteWarga", data).then ((result) => {
            if(result!=''){
                console.log("success");
            }else{
                console.log("errors");
      ...
      EditFoto = (txtkunci, txtfile) => {
        try{
            const { key, filename } = this.props.task;
            //RefWarga.child(key).remove();
            RefWarga.child(txtkunci).update({
                ...
                this.toggle4();
            upRefWarga.child(filename).delete().then(function() {
      ...
      notify(type, kunci){
        ...
        console.log(data);
        Action("updateWarga", data).then ((result) => {
            if(result!=''){
                console.log("success");
            }else{
                console.log("errors");
                ...
                if (newTask.nama.length && newTask.nik.length && newTask.nohp.length && newTask.email.length) {
