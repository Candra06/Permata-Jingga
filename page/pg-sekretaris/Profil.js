...
class Profil extends Component {
  ...
  state = ({  email: '', tempemail : '', pwlama:'', kunci: '', passwordlama:'', passwordbaru:'', konfirmasipasswordbaru:'', htgemail : 0})
  
  componentDidMount() {
    console.log(localStorage.getItem("kodeuser"))
    RefUser.orderByChild("kd_user").equalTo(localStorage.getItem("kodeuser")).on('value', snap => {
      const tasks = [];
      snap.forEach(shot => {
